...
_clState *initCl(unsigned int gpu, char *name, size_t nameSize, algorithm_t *algorithm)
{
  ...
  // Lyra2TDC
  else if (cgpu->algorithm.type == ALGO_LYRA2TDC && !cgpu->opt_tc) {
	  size_t glob_thread_count;
	  long max_int;
	  unsigned char type = 0;
	  ...
	  // if TC * scratchbuf size is too big for memory... reduce to max
	  if ((glob_thread_count * LYRA2TDC_SCRATCHBUF_SIZE) >= (uint64_t)cgpu->max_alloc) {
		  ...
		  * Adapt the memory setting. */
		  ...
		  switch (type) {
			  //raw intensity
		  case 2:
			  while ((glob_thread_count * LYRA2TDC_SCRATCHBUF_SIZE) > (uint64_t)cgpu->max_alloc) {
				  --glob_thread_count;
			  }
			  ...
			  //x intensity
		  case 1:
			  glob_thread_count = cgpu->max_alloc / LYRA2TDC_SCRATCHBUF_SIZE;
			  max_int = glob_thread_count / clState->compute_shaders;
  ...
  if (algorithm->rw_buffer_size < 0) {
      ...
      * 80 bytes only. */
      ...
      // scrypt/n-scrypt
	...
	else if (algorithm->type == ALGO_LYRA2TDC) {
		/* The scratch/pad-buffer needs 32kBytes memory per thread. */
		bufsize = LYRA2TDC_SCRATCHBUF_SIZE * cgpu->thread_concurrency;
		buf1size = 4 * 8 * cgpu->thread_concurrency; //matrix
		...
		* 80 bytes only. */
		...
		applog(LOG_DEBUG, "lyra2TDC buffer sizes: %lu RW, %lu RW", (unsigned long)bufsize, (unsigned long)buf1size);
		// scrypt/n-scrypt
	}
	else {
